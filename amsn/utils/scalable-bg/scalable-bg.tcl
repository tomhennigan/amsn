...
snit::type scalable-bg {
	...
	method BuildImage { } {
		...
		if {
			...
			#Resize left section:
			$left configure -width $options(-w) -height [expr [image height $src] - $options(-n) - $options(-s)]
			$left blank
			...
			$centre configure -width [expr [image width $src] - $options(-w) - $options(-e)] -height [expr [image height $src] - $options(-n) - $options(-s)]
			$centre blank
			$centre copy $src -from $options(-w) $options(-n) [expr [image width $src] - $options(-e)] [expr [image height $src] - $options(-s)] -to 0 0
			if { $options(-resizemethod) == "scale" } {
				::CxImage::Resize $centre [expr $options(-width) - $options(-w) - $options(-e)] [expr $options(-height) - $options(-n) - $options(-s)]
			}
	
			#Resize right section:
			$right configure -width $options(-e) -height [expr [image height $src] - $options(-n) - $options(-s)]
			$right blank
			...
			$bottom blank
			$bottom copy $src -from $options(-w) [expr [image height $src] - $options(-s)] [expr [image width $src] - $options(-e)] [image height $src]
			if { $options(-resizemethod) == "scale" } {
				::CxImage::Resize $bottom [expr $options(-width) - $options(-w) - $options(-e)] $options(-s)
			}
			...
			#NE corner
			$base copy $src -from [expr [image width $src] - $options(-e)] 0 [image width $src] $options(-n) -to [expr $options(-width) - $options(-e)] 0
			#Left border
			$base copy $left -to 0 $options(-n) $options(-w) [expr $options(-height) - $options(-s)]
			#Centre
			$base copy $centre -to $options(-w) $options(-n) [expr $options(-width) - $options(-e)] [expr $options(-height) - $options(-s)]
			#Right border
			$base copy $right -to [expr $options(-width) - $options(-e)] $options(-n) $options(-width) [expr $options(-height) - $options(-s)]
			#SW corner
			$base copy $src -from 0 [expr [image height $src] - $options(-s)] $options(-w) [image height $src] -to 0 [expr $options(-height) - $options(-s)]
			#Bottom border
			$base copy $bottom -to $options(-w) [expr [image height $base] - $options(-s)] [expr $options(-width) - $options(-e)] $options(-height)
			#SE corner
			$base copy $src -from [expr [image width $src] - $options(-e)] [expr [image height $src] - $options(-s)] [image width $src] [image height $src] -to [expr $options(-width) - $options(-e)] [expr $options(-height) - $options(-s)]
		}
	}
