...
if (mysql_num_rows($query) === 1) {
  ...
  if (mysql_num_rows($answers) > 1) {
    echo '<div>';
    echo '<h3 class="polls">'.POLLS_SIDE.'</h3>';
    echo '<div class="block_right_top"></div>';
    echo '<div class="block_right_content">';
    echo '<form action="'.inc.'poll_vote.php" method="post">';
    echo '<ul class="poll">';
    echo '<li><h4 class="poll_Q">'.trans($poll['id'], 'poll_question', $poll['question']).'</h4></li>';
    for ($i = 0; $row = mysql_fetch_row($answers); $i++) {
      echo '<li style="text-align: left">';
      echo '<input type="radio" value="'.$row[0].'" name="poll_answer" id="answer'.$i.'" /><label for="answer'.$i.'">'.trans($row[0], 'poll_answer', $row[1]).'</label>';
      echo '</li>';
    }
    echo '</ul>';
    echo '<div style="text-align: center; margin-top: 5px">';
    echo '<input type="submit" value="'.POLL_SIDE_VOTE.'" />';
    echo '<input type="hidden" name="idf" value="'.$poll['id'].'" /><br/>';
    echo '<a href="poll_results.php">'.POLL_SIDE_RESULTS.'</a>';
    echo '</div>';
    echo '</form>';
    echo '</div>';
