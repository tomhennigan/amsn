namespace eval ::winskin {
	...
	proc Init { dir } {
		...
		set ::winskin::configlist [list \
			[list bool "Ilma tiitelriba ja servadeta" removeborder] \
			[list bool "Lisanupud sees" addbuttons] \
			[list bool "Uus suurus paremalt ülalt/paremale alla" resizetopright] \
			[list bool "Kerimisriba peidetud" hidescroll] \
			[list bool "Menüüriba peidetud" hidemenu] \
			[list bool "Ilma ülaosata (olek, postkast)" removetop] \
			[list bool "Ilma ikoonideta ridade alguses (töötab 0.95b või uuemate versioonidega)" removeicons] \
			[list bool "Ilma taandeta kontaktiridade alguses (töötab 0.95b või uuemate versioonidega)" removespace] \
			[list bool "Ilma olekumärketa kontaktiridade lõpus (töötab 0.95b või uuemate versioonidega)" removestates] \
			[list bool "Alustagi WinSkini rüüga (sisse logimisel)" startskinned] \
			[list bool "Alati pealmine (saab maha võtta ainult Windowsis)" topmost] \
			[list str "Tiitelriba kõrgus pikslites (ei ole soovitatav muuta)" titleheight] \
		]

		if {$::tcl_platform(platform) == "windows"} {
			set ::winskin::config(usewinspecs) 1
			set ::winskin::configlist [linsert $::winskin::configlist 0 \
				[list bool "Kasuta Windowsile omast koodi" usewinspecs] \
			]
		}
