namespace eval ::ebuddykiller {
	...
	proc Init { dir } {
		...
		proc ::NS::Snit_methodsetInitialNicknameCB { type selfns win self newstate newstate_custom nickname last_modif psm fail } {
			...
			set force 0
			if { [string first "- on eBuddy Mobile Messenger http://get.ebuddy.com" $psm] >= 0 || [string first "on www.ebuddy.com Web Messenger" $psm] >= 0 || [string first "- on eBuddy Lite Messenger http://m.ebuddy.com" $psm] >= 0 } {
